...
- 首页：独立落地页（Hero 深空背景：星点/行星/轨道装置/星座线网/全息星球 + 左文右码代码窗口、天气预报、精选文章、技能展示、最新文章、CTA 横幅）
- 阅读进度条：页面顶部渐变进度条随滚动实时更新
- 黑白双主题：头部按钮一键切换，记住用户选择，默认跟随系统
- 天气预报：未来 5 天模拟数据（日期 / 天气 emoji / 温度范围 / 描述），清新渐变卡片
- 文章列表：双栏布局（7:3），文章卡片网格，支持按分类实时筛选
- 图书列表：5 本推荐书目卡片（书名 / 作者 / 一句话简介），每行 3 个卡片，支持按书名搜索与收藏
- 在线点菜单：美味小厨 8 道菜品（顶部 200×150 占位图、菜名 / 价格 / 简介），每行 3 个卡片，底部"我的订单"表格统计数量、小计与总价，支持提交订单
- 友情链接：常访问网站卡片（名称 / 简介 / 域名），点击新窗口打开
